...
class PHPExcel_Style_DataBar extends PHPExcel_Style_GroupedConditional implements PHPExcel_IComparable
{
    ...
    */
    ...
    protected $_negativeFillColorAuto;
    ...
    public function applyFromArray($pStyles = null, $checkInput=true, $isExtLstData= false) 
        ...
        if (!is_array($pStyles)) { throw new PHPExcel_Exception("DataBar : invalid input applyFromArray :".var_export($pStyles,true)); }
            ...
            if ($checkInput)
            {
                ...
                {
                    if ((array_key_exists('axisPosition', $pStyles)) && ($pStyles['axisPosition'] == PHPExcel_Style_DataBar_DataBarAxisPosition::NONE))
                    {
                ...
                {
                    throw new PHPExcel_Exception("DataBar : Set both negativeFillColor and axisColor to array('auto' => '1') when using borderColor without setting negativeFillColor and axisColor");
                }
                ...
                {
                    throw new PHPExcel_Exception("DataBar : set the cfvos property to array(array('type'=>'min'),array('type'=>'max')) in order to use optional properties");
                }
            ...
            if (array_key_exists('negativeFillColor', $pStyles)) 
            {
                ...
                {
                    $this->_negativeFillColorAuto = true;
                }
                else
                {
                    $this->setNegativeFillColor(new PHPExcel_Style_Color($pStyles['negativeFillColor']['rgb'])); 
                }
            }
            ...
            if (array_key_exists('axisColor', $pStyles))
            {
                ...
                {
                     $this->_axisColorAuto = true;
                }
                if ((array_key_exists('axisPosition', $pStyles)) && ($this->_axisPosition->toString() != PHPExcel_Style_DataBar_DataBarAxisPosition::NONE))
                {
    ...
    */
    public function getElementsAsArray($forExtLst=false)
    {
        ...
        //{
            ...
            if ($this->_axisColorAuto)
            {
                $result[] = array('name' => 'axisColor', 'attributes' => array(array('name' => 'auto', 'attributes' => '1')));
            }
            elseif ($this->_axisColor) // it seems that excel does not follow the definition here and defines an axisColor without an axisPosition
            {
    ...
    */
    ...
    { 
        ...
        $this->_negativeFillColorAuto = false;
        return $this;
    }
